...
class Connection:
	def __init__(self, value):
		self.ip_connection = value
		self.next = None

	def get_ip_connection(self):
		return self.ip_connection

	def get_next(self):
		return self.next

	def set_ip_connection(self, new_value):
		self.ip_connection = new_value

	def set_next(self, next_connection):
		self.next = next_connection
...
class Connection_List:
	...
	def remove(self, item):
		...
		it_next = self.first
		found = False
		while(not found and it_next != None):
			if(it_next.get_ip_connection() == item):
				found = True
				if(it == None):
					self.first = it_next.get_next()
				else:
					it.set_next(it_next.get_next())
			else:
				it = it_next
				it_next = it_next.get_next()
		self.size -= 1
		return found	
	...
	def search(self, item):
		it = self.first
		found = False
		while(not found and it != None):
			if(it.get_ip_connection() == item):
				found = True
			else:
				it = it.get_next()
		return found
	...
	def print_list(self):
		...
		while(it != None):
			print("Ip connection: "+it.get_ip_connection())
			found = True
			it = it.get_next()
...
# We need to create the shared data structure to store the "Lista de Conexiones"
